...
void Verlet(particle* prev_part, particle* output_part, Vec3d force, float dt)
{
	...
	drag_force.y = 0.3f * prev_part->speed.x * prev_part->speed.x * ((prev_part->speed.x < 0) ? 1 : -1);
	LOG("%f", (0.5f * 1.225f * (3.14159265f / 2.0f) * 0.47f))

	Vec3d new_acc;
	new_acc.x = (force.x + drag_force.x) / prev_part->mass ;
	new_acc.y = (force.y + drag_force.y) / prev_part->mass + prev_part->acc.y;
